...
impl M68K {
    pub fn new(bus: M68KBus) -> Self {
        let ssp: u32 = unsafe {
            ...
            u32::from(ms[0]) << 24 | u32::from(ms[1]) << 16 | u32::from(ls[0]) << 8 | u32::from(ls[1])
        };
        let pc: u32 = unsafe {
    ...
    fn read_u32(&mut self, addr: u32) -> u32 {
        ...
        if result1.0.is_some() { todo!(); }
        let result2 = unsafe { self.bus.borrow().read(addr & 0xFFFFFFFE) };
        if result2.0.is_some() { todo!(); }
        else {
            u32::from(result1.1[0]) << 24 | u32::from(result1.1[1]) << 16 | u32::from(result2.1[0]) << 8 | u32::from(result2.1[1])
        }
    }
}
            ...
            0 => if insn & 0x0100 == 0 {
                match insn & 0x0E00 {
                    0x0200 => {
                        ...
                        let ea = M68KEffectiveAddress::new(size, (insn & 0x0038) >> 3, insn & 0x0007);
